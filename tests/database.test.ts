...
test('database references are complete and all 48 enemy variants produce valid intents',()=>{
 assert.equal(Object.keys(ENEMIES).length,48);for(const c of CAMPAIGN)for(const s of c.stages){assert.ok(ENEMIES[s.enemy]);assert.ok(s.hp>0);}
 ...
 assert.equal(TALENTS.length,50);for(const t of TALENTS)if(t.requires)assert.ok(TALENTS.find(x=>x.id===t.requires));
